...
mod itx_isolation {
    ...
    async fn spacing_doesnt_matter(mut runner: Runner) -> TestResult<()> {
        let tx_id = runner.start_tx(5000, 5000, Some("Repeatable Read".to_owned()), None).await?;
        runner.set_active_tx(tx_id.clone());

        let res = runner.commit_tx(tx_id).await?;
        assert!(res.is_ok());

        let tx_id = runner.start_tx(5000, 5000, Some("RepeatableRead".to_owned()), None).await?;
        runner.set_active_tx(tx_id.clone());
